...
--  DÙNG CHUNG bảng public.users sẵn có của DA project.
...
-- ---------- 0. ĐẢM BẢO users.username là UNIQUE/PK ----------
...
create table if not exists public.evaluations (
  ...
  period          text not null references public.kpi_periods (period),
...
create index if not exists idx_objectives_eval on public.objectives (evaluation_id);
create index if not exists idx_evaluations_user on public.evaluations (username);
create index if not exists idx_evaluations_period on public.evaluations (period);
create index if not exists idx_kpi_profiles_mgr on public.kpi_profiles (manager_username);
...
create or replace view public.kpi_users as
  select username, ho_va_ten, role as da_role, mien, bu
  from public.users;

grant select on public.kpi_users to authenticated, anon;
...
as $$
  select exists (
    select 1 from public.kpi_profiles
    where username = current_username() and is_admin
  );
$$;
...
as $$
  select exists (
    select 1 from public.kpi_profiles
    where username = target and manager_username = current_username()
  );
$$;
...
drop trigger if exists trg_eval_touch on public.evaluations;
...
drop policy if exists kpi_profiles_select on public.kpi_profiles;
create policy kpi_profiles_select on public.kpi_profiles
  for select to authenticated using (true);

-- Chỉ admin được sửa quan hệ quản lý / quyền admin
drop policy if exists kpi_profiles_admin_write on public.kpi_profiles;
create policy kpi_profiles_admin_write on public.kpi_profiles
  for all to authenticated
  using (kpi_is_admin())
  with check (kpi_is_admin());

-- ---------- RLS: kpi_periods ----------
drop policy if exists periods_select on public.kpi_periods;
create policy periods_select on public.kpi_periods
  for select to authenticated using (true);

drop policy if exists periods_admin_all on public.kpi_periods;
create policy periods_admin_all on public.kpi_periods
  for all to authenticated
  using (kpi_is_admin())
  with check (kpi_is_admin());
...
create policy obj_write on public.objectives
  for all to authenticated
  using (
    exists (
      select 1 from public.evaluations e
